...
def make_model(tweets):
    with gr.Blocks(theme="Base") as interface:
        ...
        def change_slider(choice,previous_val):
            ...
            histo = create_histogram_topk(topEntities(current_val, count_entities(tweets,choice.lower())), choice.lower())
            if choice:
                return {
        ...
        radio_top = gr.Radio(["Hashtags", "Arobases", "Users"], label="Top K", info="Cochez la case si vous souhaitez avoir le top K renseignés.")
        
        slider_hashtags = gr.Slider(visible=False)
        plot_hashtags = gr.Plot(visible=False)
        
        radio_top.change(fn=change_slider, inputs=[radio_top, slider_hashtags], outputs=[slider_hashtags, plot_hashtags])
